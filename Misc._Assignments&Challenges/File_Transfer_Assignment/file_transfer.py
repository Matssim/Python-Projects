...
class ParentWindow(Frame):
    def __init__(self, master):
        ...
        #Positions source in GUI using tkinter grid() padx and pady are the same as
        ...
        #The section below automatically transfer files that have been modified in
        #the last 24 hours
        #Establishes the filepath of the current working directory
        cwd_path = Path.cwd()
        #Establishes the source folder path
        source_path = os.path.join(cwd_path, "Customer Source")
        #Establishes the destination folder path
        ...
        #Establishes the function for each file in the source folder
        for i in sourcefolder_file:
            #Creates a full file path for each file in the source folder
            sourcefile_path = os.path.join(source_path, i)
            #Extracts the timestamp from when the files where last modified
            sourcefile_modtimestamp = os.path.getmtime(sourcefile_path)
            #Converts the modification timestamp to datetime format
            sourcefile_modtime = datetime.utcfromtimestamp(sourcefile_modtimestamp)
            #Extracts the current time
            currenttime = datetime.now()
            #Calculates the time lapsed from the last modification and converts it to
            #total seconds
            sourcefile_modage = timedelta.total_seconds(currenttime - sourcefile_modtime)
            #Creates a function that runs for files that have been modified in the last
            #24 hours (86400 seconds)
            if sourcefile_modage >= 86400:
                #moves the relevant files from the source folder to the destination folder
